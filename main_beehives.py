...
print(f'serial data,{datetime.now() - start_time}')
prev_time = datetime.now()

nb_img = 20
path_to_img = capture_images(nb_img)
print(f'capture {nb_img} images,{datetime.now() - prev_time}')
prev_time = datetime.now()

resize_img(path_to_img)
print(f'resize images,{datetime.now() - prev_time}')
prev_time = datetime.now()

path_to_gif_file = convert_jpg_to_gif(path_to_img)
print(f'convert jpg to gif,{datetime.now() - prev_time}')
prev_time = datetime.now()

gif_url = upload_to_imgur(beehive_id, path_to_gif_file)
print(f'upload gif to imgur,{datetime.now() - prev_time}')
prev_time = datetime.now()

upload_to_spreadsheet(beehive_id, serial_data, gif_url)
print(f'upload data to spreadsheet,{datetime.now() - prev_time}')
prev_time = datetime.now()
